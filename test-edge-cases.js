...
const TESTS = [
  ...
  { name: "HEALTH-02: Root serves current app.js v5.0.0", fn: () => checkVersion() },
  ...
  { name: "EDGE-01: GET /api/chat → 401 or 405", fn: () => get("/api/chat", null) },
...
async function run() {
  ...
  for (let i = 0; i < TESTS.length; i++) {
    ...
    try {
      ...
      if (t.name.startsWith("SEC-")) {
        ...
        if (t.name.includes("v5.0.0")) ok = result.version === "5.0.0";
        ...
        if (t.name.includes("401 or 405")) ok = result.status === 401 || result.status === 405;
  ...
  fs.writeFileSync("/home/user/test-report.json", JSON.stringify({ passed, failed, results }, null, 2));
  console.log("\nReport saved.");
}
